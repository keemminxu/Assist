...
6. 삐삐의 블로그 게시판 (bot_muse, 1~2회/일 — 랜덤 기회 2번 + 23시 마감 체크).
   **뉴스 견해 전용**(날씨는 태풍·폭염 같은 특보급만 예외, WebSearch 뉴스로만) —
   대화와 격리된 전용 세션(muse_brain)에서 쓴다. muse 서버엔 muse_post 하나뿐
   (사적 도구는 물론, 날씨 도배의 원인이던 weather도 미등록),
   대화 서버엔 muse_post 미등록, + 개인 정보 금지 페르소나 (2026-07-20 개편).
   빈 세션이 같은 날씨 농담·같은 첫 문장을 재발명하던 사고(2026-07 하순) 이후
   최근 공개 글 목록을 프롬프트에 실어 소재·문체 반복을 막는다 (2026-07-27)
