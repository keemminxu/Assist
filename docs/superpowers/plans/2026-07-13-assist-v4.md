...
> **구현 노트 (리뷰 반영, 위 참조 코드보다 우선):**
> 1. `test_update_moves_event`의 PATCH 경로 어서션은 `/calendar/v3/calendars/...`가 맞음 (httpx base_url 경로 병합 — 계획 원문 오류).
> 2. `update()` 종일↔시간형 전환 시 반대 필드를 명시적으로 null 클리어해야 함 (`"date": None` / `"dateTime": None`) — Google PATCH merge 의미론상 공존 시 400.
> 3. `update()`에서 new_start만 주어진 이동은 원본 이벤트의 duration을 보존 (무조건 +1h 아님; 원본이 종일이면 +1h 폴백).
> 4. `_list()`는 nextPageToken 페이지네이션 필수 (maxResults 250/페이지) — 잘림 시 AmbiguousMatch 안전장치가 무력화되어 오삭제 위험.
> 테스트 4개 추가: duration 보존, 종일→시간형 date null, 페이지네이션, 4xx→GcalError (총 15개).
